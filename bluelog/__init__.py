...
def register_shell_context(app):
    # 这段代码定义了一个名为 register_shell_context 的函数，用于在 Flask 应用中注册一个 shell 上下文处理器。
    # 该处理器使得在使用 flask shell 命令启动交互式 shell 时，预先导入特定的对象，方便开发者直接使用，而无需每次手动导入
...
def register_template_context(app):
    # 上下文处理器装饰器：在函数内部，使用 @app.context_processor 装饰器定义了一个名为 make_template_context 的函数。
    # 这个装饰器的作用是注册一个上下文处理器，使其返回的变量在所有模板中全局可用。
